...
def _reexec_in_xterm_if_needed() -> None:
    """Re-exec under xterm when visualizer.use_xterm is set in the config.

    Guarded by WEATHER_VIZ_IN_XTERM to prevent re-exec loops. Also used by
    the launch file prefix approach, which sets the flag directly.
    """
    import shutil
    import sys

    if os.environ.get('WEATHER_VIZ_IN_XTERM'):
        return
    if not shutil.which('xterm'):
        return

    config_path = os.environ.get('WEATHER_CONFIG_PATH')
    if not config_path:
        # Best-effort: locate the installed package config
        try:
    ...
    try:
        ...
        use_xterm = bool(cfg.get('visualizer', {}).get('use_xterm', False))
    except Exception:
        return
    ...
    env = os.environ.copy()
    env['WEATHER_VIZ_IN_XTERM'] = '1'
    os.execvpe('xterm', ['xterm', '-e'] + sys.argv, env)


def main(args=None):
    _reexec_in_xterm_if_needed()
