...
for publishing and `<domain_id>/<topic>/<dds_type>/*` for subscribing. Set it when the peer is not a
ROS node and should not have to construct that. It makes `domain_id` irrelevant for the mapping, and
on `ros2_to_zenoh` suppresses the liveliness token, since the mapping no longer describes an
`rmw_zenoh_cpp` topic.
